...
function AddSlider(num){
  ...
  for(var i=0;i<num;i++)
  {
    var a='<span>'+robot.link[robot.linkNumArray[i]].RANGE.min+'</span>\
                    <input id="ex'+i+'"type="text" serier="'+i+'"data-slider-min="'+robot.link[robot.linkNumArray[i]].RANGE.min+'" data-slider-max="'+robot.link[robot.linkNumArray[i]].RANGE.max+'" data-slider-step="0.01" data-slider-value="0" />\
                    <span>'+robot.link[robot.linkNumArray[i]].RANGE.max+'</span>\
    ...
    $("#ex" + i + "SliderVal").change(function(event) {
      ...
      mySlider.slider('setValue', parseFloat($('#ex' + j + 'SliderVal').val()));
      LinkValuearr[j] = parseFloat($('#ex' + j + 'SliderVal').val());
      changeArray(Oldarr, LinkValuearr);
    });
  }
  $(".InverseBtn").click(function(){
      var InputArray=$("div>p>input");
      getValue(parseFloat(InputArray[0].value),parseFloat(InputArray[1].value),parseFloat(InputArray[2].value));
   });
...
function updateobj(){
  ...
  // line.geometry.vertices.push(Point);
